public class KnightBoard{
  ...
  */
  ...
  private boolean solveH(int row, int col, int level){
    ...
    if (row < 0 || col < 0 || row >= board.length || col >= board[row].length){
      return false; //if any moves go out of bounds return false
    }
  ...
  */
  ...
  public int countH(int row, int col, int level){
    ...
    if (row < 0 || col < 0 || row >= board.length || col >= board[row].length){
      return 0; //if any moves go out of bounds return 0
    }

    if (board[row][col] != 0) return 0; //if knight has already been to that position

    int count = 0;
    for (int i = 0; i < diff.length; i++){ //loops through moves
      board[row][col] = level; //moves knight
